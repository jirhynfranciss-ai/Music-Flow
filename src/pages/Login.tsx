...
import { Mail, Lock, Eye, EyeOff, Disc3 } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const { login, loginWithGoogle, isAuthenticated, theme } = useStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      setError('Invalid email or password. Try: admin@musicflow.com or user@example.com with any 6+ char password');
    }
  };
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center p-12 z-10">
          ...
          <p className="text-xl text-white/80 text-center max-w-md">
            Stream millions of songs from your favorite artists. Green Day, Nickelback, The Weeknd, and more.
          </p>
      ...
      <div className={`w-full lg:w-1/2 flex items-center justify-center p-8 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="w-full max-w-md">
          ...
          <p className={`mb-8 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Sign in to continue to MusicFlow
          </p>
          ...
          <p className={`mt-4 text-center text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
            Demo: admin@musicflow.com / user@example.com (any 6+ char password)
          </p>
        </div>
      </div>
    </div>
